...
public class BasePhysicsEngine(ObjectResource ObjectsReferents) : IPhisicsEngine
{
    ...
    public override bool AreThereCollisions(Controller body, Point2 move)
    {
        ...
        foreach(var part in body.Entity.Body.Elements){
            ...
            foreach(var otherObject in ObjectsReferents.GetAllObjects()){
                ...
                if (otherObject.Body.Elements.Any(x=>x.Value && x.Key.Plus(otherObject.AbsolutePosition.Plus(otherObject.Body.Position))==absolutePositionPart))
                    return true;
            }
        }
        return false;
